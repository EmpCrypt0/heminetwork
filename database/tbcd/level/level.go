...
func (l *ldb) BlockHeaderGenesisInsert(ctx context.Context, wbh *wire.BlockHeader, height uint64, diff *big.Int) error {
	...
	// If this is the real genesis block, set starting cdiff to embedded difficulty
	cdiff := blockchain.CalcWork(wbh.Bits)

	// If diff is supplied *and* not set to zero, then use it instead (external header mode)
...
func (l *ldb) BlockHeadersRemove(ctx context.Context, bhs *wire.MsgHeaders, tipAfterRemoval *wire.BlockHeader, upstreamStateId *[32]byte) (tbcd.RemoveType, *tbcd.BlockHeader, error) {
	...
	tipAfterRemovalParsed := tipAfterRemoval
	...
	if len(headersParsed) > 1 {
		for i := len(headersParsed) - 1; i > 0; i-- {
			...
			if !bytes.Equal(prevHash[:], prevHashCalc[:]) {
				return tbcd.RTInvalid, nil,
					fmt.Errorf("block headers remove: header with hash %s does not connect to previous"+
						" header to remove with hash %s", headersParsed[i].BlockHash().String(), prevHashCalc.String())
			}
		}
	}
	...
	for i := 0; i < len(headersParsed); i++ {
		...
		if tipAfterRemovalHash.IsEqual(&hash) {
			return tbcd.RTInvalid, nil,
				fmt.Errorf("block headers remove: cannot remove header with has %s when that is supposed to be"+
	...
	// TODO: Check that we are not removing the first known block in the database
	...
	for i := 0; i < len(fullHeadersFromDb); i++ {
		...
		if !expectedHash.IsEqual(&dbHeaderReconstitutedHash) {
			// Recalculated hash of header  from database doesn't match header of block, this should also be
			// impossible but extra sanity.
			return tbcd.RTInvalid, nil,
	...
	// Check that all of the headers were removed correctly
	for i := 0; i < len(headersParsed); i++ {
		bh := headersParsed[i].BlockHash()
		deletedHeaderFromDb, err := l.BlockHeaderByHash(ctx, &bh)
		if err == nil {
			dh := *deletedHeaderFromDb
			panic(fmt.Sprintf("Block by hash %s should have been deleted, but it is still in database!", dh.BlockHash().String()))
		}
	}
