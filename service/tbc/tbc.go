...
func (s *Server) insertGenesisHeader(ctx context.Context, genesisHeader *wire.BlockHeader, height uint64, diff *big.Int) error {
	gbh, err := header2Array(genesisHeader)
	if err != nil {
		return fmt.Errorf("serialize genesis block header: %w", err)
	}
	if err = s.db.BlockHeaderGenesisInsert(ctx, gbh, height, diff); err != nil {
...
func (s *Server) insertGenesis(ctx context.Context, height uint64, diff *big.Int) error {
	...
	err := s.insertGenesisHeader(ctx, &s.chainParams.GenesisBlock.Header, height, diff)
	if err != nil {
		return err
	}
...
func (s *Server) TxById(ctx context.Context, txId *chainhash.Hash) (*wire.MsgTx, *chainhash.Hash, error) {
	...
	if s.cfg.ExternalHeaderMode {
		return nil, nil, errors.New("cannot call tx by id on TBC running in External Header mode")
	}
	...
	// XXX Track which blocks are canonical somehow (new table?), and choose the hash from
	// below that is canonical, or if no canonical block exists do not return the tx at all.
...
func (s *Server) ExternalHeaderSetup(ctx context.Context) error {
	...
	if err != nil {
		...
		if err = s.insertGenesisHeader(ctx, genesis, genesisHeight, genesisDiff); err != nil {
			return fmt.Errorf("insert genesis: %w", err)
		}
		bhb, err = s.db.BlockHeaderBest(ctx)
		if err != nil {
